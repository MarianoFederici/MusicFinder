...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), currentStructure(StructureType::HEAP)
{
    ui->setupUi(this);
...
void MainWindow::populateStructures() {
    ...
    popularityTree = new RedBlackTree(SortMode::POPULARITY);
    danceTree = new RedBlackTree(SortMode::DANCEABILITY);
    energyTree = new RedBlackTree(SortMode::ENERGY);
    durationTree = new RedBlackTree(SortMode::DURATION);
    for (const auto &s : songs) {
        ...
        popularityTree->insert(s);
        danceTree->insert(s);
        energyTree->insert(s);
        durationTree->insert(s);
    }
}

std::vector<Song> MainWindow::getTopSongs(SortMode mode, int count) const {
    if (currentStructure == StructureType::HEAP) {
        const MaxHeap* heapPtr = nullptr;
        switch (mode) {
        case SortMode::POPULARITY: heapPtr = &popularityHeap; break;
        case SortMode::DANCEABILITY: heapPtr = &danceHeap; break;
        case SortMode::ENERGY: heapPtr = &energyHeap; break;
        case SortMode::DURATION: heapPtr = &durationHeap; break;
        }
        ...
        case SortMode::DANCEABILITY: treePtr = danceTree; break;
        case SortMode::ENERGY: treePtr = energyTree; break;
        case SortMode::DURATION: treePtr = durationTree; break;
        }
        return treePtr ? treePtr->getTopN(count) : std::vector<Song>();
    }
}
...
void MainWindow::displaySongs(SortMode mode, bool reset) {
    ...
    int totalAvailable = static_cast<int>(songs.size());
...
void MainWindow::on_btnHeaps_clicked() {
    currentStructure = StructureType::HEAP;
    displaySongs(activeMode, true);
}

void MainWindow::on_btnRedBlack_clicked() {
    currentStructure = StructureType::RED_BLACK_TREE;
    displaySongs(activeMode, true);
}
